...
export async function initTikTokDirectPost({accessToken, videoSize, caption="", privacyLevel}) {
  ...
  if (!chosen) throw new Error("TikTok tidak mengembalikan privacy_level_options");
  if (caption.length > 2200) throw new Error("Caption TikTok melebihi 2200 karakter");

  return postJson(`${API}/post/publish/video/init/`, accessToken, {
    post_info:{
      title:caption,
    ...
    source_info:{
      ...
      chunk_size:Math.min(64 * 1024 * 1024, Math.max(5 * 1024 * 1024, videoSize)),
      total_chunk_count:Math.ceil(videoSize / Math.min(64 * 1024 * 1024, Math.max(5 * 1024 * 1024, videoSize)))
    }
  });
}
